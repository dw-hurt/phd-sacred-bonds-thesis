...
.EXAMPLE
    .\Add_Integration_Roadmap_To_GitBook_FIXED.ps1
    # Runs with default settings

.EXAMPLE
    .\Add_Integration_Roadmap_To_GitBook_FIXED.ps1 -RepoPath "D:\PhD\thesis" -AutoCommit:$false
...
# Try multiple possible source filenames
$possibleSourceFiles = @(
    "Integration_Roadmap_2025-11-29.md",
    "COMPLETE_Integration_Roadmap_2025-11-29.md"
)

$sourceFile = $null
foreach ($filename in $possibleSourceFiles) {
    if (Test-Path $filename) {
        $sourceFile = $filename
        Write-Success "Found source file: $sourceFile"
        break
    }
}

if (-not $sourceFile) {
    Write-Fail "Source file not found. Tried:"
    foreach ($filename in $possibleSourceFiles) {
        Write-Info "  - $filename"
    }
    Write-Host "`nPlease ensure the Integration Roadmap file is in the current directory" -ForegroundColor Yellow
    Write-Host "You can download it from:" -ForegroundColor Yellow
    Write-Host "  computer:///mnt/user-data/outputs/Integration_Roadmap_2025-11-29.md" -ForegroundColor Cyan
    exit 1
}
