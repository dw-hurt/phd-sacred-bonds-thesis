...
.NOTES
    ...
    Date: 2025-11-25
    Version: 2.0 (Fixed regex syntax)
#>
...
if (-not $GitBookUrl) {
    ...
    if (Test-Path $ReadmeFile) {
        ...
        if ($content -match '(https?://[^\s]+\.gitbook\.io/[^\s\)]+)') {
            $GitBookUrl = $matches[1]
...
if (-not $GitBookUrl) {
    ...
    try {
        ...
        if ($remotes -match '(https?://[^\s]+gitbook[^\s]+)') {
            $GitBookUrl = $matches[1]
...
try {
    ...
    # This is a simplified extraction - actual GitBook structure may vary
    $publishedLinks = @()
    
    # Try to extract from navigation elements
    $navPattern = '<a[^>]+href=["'']([^"'']+)["''][^>]*>([^<]+)</a>'
    $html | Select-String -Pattern $navPattern -AllMatches | ForEach-Object {
        $_.Matches | ForEach-Object {
            ...
            # Filter to only include content links (not external/navigation links)
            if ($href -notmatch '^(http|#|javascript)' -and $title -notmatch '^(Home|Search|Menu)$') {
    ...
    Write-Success "Extracted $($publishedLinks.Count) navigation links from published GitBook"
    ...
    $publishedFirmanLinks = $publishedLinks | Where-Object { 
        $_.Path -like '*firman*' -or $_.Title -like '*firman*' 
    }
